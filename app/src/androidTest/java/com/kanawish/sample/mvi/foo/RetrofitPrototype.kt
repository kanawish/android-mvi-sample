 ...
 */
fun main() {
    // Creates a bogus toothpick scope, to validate if our module setup above works.
    val scope = Toothpick
        .openScope("MAIN")
    ...
    val mockSequence = tasksRestApi.getTasks()
        ...
        .onErrorReturn { TasksState(emptyList(), FilterType.ANY, SyncState.ERROR(it)) }

    // In the real code, we'll build an Observable<Intent<T>>
    val disposable = mockSequence
        .toList() // Useful for this prototype, toList() makes our Observable blocking.
        .subscribe { resultsList ->
            resultsList.forEach { println(it) } // Dump all the results to the console.
        }

    // Since it's a blocking chain, this will always be true below.
